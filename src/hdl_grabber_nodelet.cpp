...
// AUGUSTO DELETES THESE LINES. GPS IS PUBLISHED BY OTHER NODE            
//             imu_vel_pub_ = node.advertise<sensor_msgs::Imu>("/velodyne/imu_data", 10);
//             if (is_enable_gps_) {
//                 nav_fix_pub_ = node.advertise<sensor_msgs::NavSatFix>("/velodyne/gps", 20);
//             } else {
//                 nmea_pub_ = node.advertise<nmea_msgs::Sentence>("/velodyne/nmea_sentence", 10);            }
        ...
        bool loadSyncData(std::string fname, bool ismapping) { // ismapping or localizing{
            ...
            if (odom_file == NULL) {
                ...
                odom_data_sync_ = (st_sync_data_ *) realloc(odom_data_sync_,
                                                            (icount * iodom_samples_ * sizeof(st_sync_data_)));
            ...
            
            {
                ...
                while (nums > 0) {
                    ...
                    if (iodom_samples_ == (icount * 100)) {
                        icount++;
                        odom_data_sync_ = (st_sync_data_ *) realloc(odom_data_sync_,
                                                                    (icount * 100 * sizeof(st_sync_data_)));


                    }
                    nums = fscanf(odom_file, "%d\t%u\t%f\t%f\t%f\t%f\t%f\t%f\t%f\t%f\t%f\t%f\n",
                                  &odom_data_sync_[iodom_samples_].sweep_counter,
                                  &odom_data_sync_[iodom_samples_].timestamp,
                                  ...
                                  &odom_data_sync_[iodom_samples_].pitch_absolute,
                                  &odom_data_sync_[iodom_samples_].roll_absolute,
                ...
                if (odom_file != NULL) {
                    ...
                    while (nums > 0) {
                        ...
                        if (iimu_samples_ == (icount * 100)) {
                            icount++;
                            imu_pose_sync_ = (st_imu_data_ *) realloc(imu_pose_sync_,
                                                                      (icount * 100 * sizeof(st_imu_data_)));
                        }
                        nums = fscanf(odom_file, "%d\t%u\t%f\t%f\t%f\t%f\t%f\t%f\t%f\t%f\t%f\n",
                                      &imu_pose_sync_[iimu_samples_].sweep_counter,
                                      &imu_pose_sync_[iimu_samples_].timestamp,
                                      &imu_pose_sync_[iimu_samples_].accx, &imu_pose_sync_[iimu_samples_].accy,
                                      &imu_pose_sync_[iimu_samples_].accz,
                                      &imu_pose_sync_[iimu_samples_].gyrox, &imu_pose_sync_[iimu_samples_].gyroy,
                                      &imu_pose_sync_[iimu_samples_].gyroz,
                                      &imu_pose_sync_[iimu_samples_].yaw, &imu_pose_sync_[iimu_samples_].pitch,
                                      &imu_pose_sync_[iimu_samples_].roll);
                ...
                if (odom_file != NULL) {
                    ...
                    for (itt = 0; itt < iodom_samples_; itt++) {
                        fscanf(odom_file, "%d\t%u\t%f\t%f\t%f\t%lf\t%d\n",
                               ...
                               &gps_data_sync_[itt].latitude, &gps_data_sync_[itt].longitude,
                               &gps_data_sync_[itt].altitude,
        ...
        bool start(void) {
            if (!is_running_) {
                ...
                } else { // if  is_live && non sync data nor buscan_imu
                    //  ROS_ERROR("VelodyneHDLDriver::runPCloudAcquisition");
                    acquisition_thread_ = new boost::thread(
                    ...
                    if (this->sync_data_) {
                        ...
                        for (ii = 0; ii < this->iodom_samples_; ii++) {
                            ...
                            if (this->odom_pose_sync_[ii].sweep_counter == (unsigned int) cloud_ptr->header.seq) {
                                ...
                                q.setRPY(this->odom_pose_sync_[ii].roll, this->odom_pose_sync_[ii].pitch,
                                         this->odom_pose_sync_[ii].yaw);
                                ...
                                qmap.setRPY(this->map_pose_sync_[ii].roll, this->map_pose_sync_[ii].pitch,
                                            this->map_pose_sync_[ii].yaw);
                ...
                if (cloud_ptr->size() > 0) {
                    ...
                    if (this->sync_data_) {
                        ...
                        for (ii = 0; ii < this->iodom_samples_; ii++) {
                            ...
                            if (this->odom_pose_sync_[ii].sweep_counter == (unsigned int) cloud_ptr->header.seq) {
                                ...
                                drive_odom_msg.pose.position.y = this->odom_pose_sync_[ii].y;
                                drive_odom_msg.pose.position.z = this->odom_pose_sync_[ii].z;
                                tf::Quaternion q = tf::createQuaternionFromRPY(this->odom_pose_sync_[ii].roll,
                                                                               this->odom_pose_sync_[ii].pitch,
                                                                               this->odom_pose_sync_[ii].yaw);
                                drive_odom_msg.pose.orientation.x = q.x();
                                drive_odom_msg.pose.orientation.y = q.y();
                                drive_odom_msg.pose.orientation.z = q.z();
                                ...
                                tf::Quaternion qmap = tf::createQuaternionFromRPY(this->map_pose_sync_[ii].roll,
                                                                                  this->map_pose_sync_[ii].pitch,
                                                                                  this->map_pose_sync_[ii].yaw);
                                drive_map_msg.pose.orientation.x = qmap.x();
                                drive_map_msg.pose.orientation.y = qmap.y();
                                drive_map_msg.pose.orientation.z = qmap.z();
                                ...
                                if (this->odom_pose_sync_[ii].timestamp > 0) {
                                    ...
                                    if (is_first_pose) {
                                        ...
                                        msf_pose_update.header.stamp = ros::Time::now();
                                        msf_pose.pose.pose.position.x =
                                                this->odom_pose_sync_[ii].x + 1.09 * cos(this->odom_pose_sync_[ii].yaw);
                                        msf_pose.pose.pose.position.y =
                                                this->odom_pose_sync_[ii].y + 1.09 * sin(this->odom_pose_sync_[ii].yaw);
                                        msf_pose.pose.pose.position.z = this->odom_pose_sync_[ii].z;
                                        msf_pose.pose.pose.orientation.x = q.x();
                                        msf_pose.pose.pose.orientation.y = q.y();
                                        msf_pose.pose.pose.orientation.z = q.z();
                    ...
                    if (sync_idx >= 0) {
                        ...
                        if (this->request_pcl_srv_.call(call_srv)) {
                            ROS_WARN_STREAM(
                                    "2 HDL driver Request PointCloud velodyne frame: " << cloud_ptr->header.seq);
                        } else {
                    ...
                    if ((diff_time * 1000) > 8) {
                        ...
                        for (ii = id_imu; ii < this->iimu_samples_; ii++) {
                            ...
                            if (this->imu_pose_sync_[ii].sweep_counter == id_sync_imu) {
                                ...
                                msg_imu.angular_velocity.y = (double) this->imu_pose_sync_[ii].gyroy;
                                msg_imu.angular_velocity.z = (double) this->imu_pose_sync_[ii].gyroz;
                                q.setRPY(this->imu_pose_sync_[ii].roll, this->imu_pose_sync_[ii].pitch,
                                         (this->imu_pose_sync_[ii].yaw - init_yaw));
                                msg_imu.orientation.w = q.w();
                                msg_imu.orientation.x = q.x();
                                msg_imu.orientation.y = q.y();
                                msg_imu.orientation.z = q.z();
            ...
            while (this->is_running_) {
                ...
                if (this->gps_mutex_.try_lock()) {
                    ...
                    if (this->gps_data_ && is_new_gps_imu_ == true) {
                        ...
                        msg_imu.linear_acceleration.x = (this->gps_data_->gyro_temp_accel_xyz[0].accel_x +
                                                         this->gps_data_->gyro_temp_accel_xyz[0].accel_y) / 2.0;
                        msg_imu.linear_acceleration.y = (this->gps_data_->gyro_temp_accel_xyz[1].accel_x +
                                                         this->gps_data_->gyro_temp_accel_xyz[1].accel_y) / 2.0;
                        msg_imu.linear_acceleration.z = (this->gps_data_->gyro_temp_accel_xyz[2].accel_x +
                                                         this->gps_data_->gyro_temp_accel_xyz[2].accel_y) / 2.0;
