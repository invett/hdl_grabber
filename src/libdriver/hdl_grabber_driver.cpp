...
const boost::asio::ip::address HDLGrabberDriver::HDL_DEFAULT_NETWORK_ADDRESS = boost::asio::ip::address::from_string ("192.168.0.200");
...
: hdl_data_ ()
 ...
 , udp_listener_gps_endpoint_ (HDL_DEFAULT_NETWORK_ADDRESS, HDL_GPSDATA_PORT)
 , source_address_filter_ ()
 , source_port_filter_ (443)
 , hdl_read_socket_service_ ()
 , hdl_read_socket_ (NULL)
 , hdl_read_gps_socket_service_ ()
...
// carlota
 ...
 , snap_sweep_xyz_signal_ ()
 , snap_sweep_xyzrgb_signal_ ()
 , snap_sweep_xyzi_signal_ ()
 , current_snap_sweep_xyz_ (new pcl::PointCloud<pcl::PointXYZ> ())
 , current_snap_sweep_xyzi_ (new pcl::PointCloud<pcl::PointXYZI> ())
 , current_snap_sweep_xyzrgb_ (new pcl::PointCloud<pcl::PointXYZRGBA> ())
 , firecount(0)
 , is_set_pause_on_(false)
 , scan_data_count_(0)
 , is_scan_correction_on_(true)
 , sweepCorrectionFile_(sweepCorrectionFile)
 , pcap_ip_port_filter_()
 , velodyne_model_(vmodel)
 , isTransformOn_(false)
...
HDLGrabberDriver::HDLGrabberDriver (const boost::asio::ip::address& ipAddress,
							 ...
							 const std::string& correctionsFile, const bool& use_gps, const bool& use_laser, HDLGrabberDriver::VELODYNE_MODEL vmodel)
: hdl_data_ ()
 , gps_data_ ()
 , udp_listener_endpoint_ (HDL_DEFAULT_NETWORK_ADDRESS, port)
 , udp_listener_gps_endpoint_ (HDL_DEFAULT_NETWORK_ADDRESS, port_gps)
 , source_address_filter_ ()
 , source_port_filter_ (443)
 , hdl_read_socket_service_ ()
 , hdl_read_socket_ (NULL)
		   ...
		   BOOST_FOREACH(boost::property_tree::ptree::value_type &px, points)
		   {
			 if (px.first == "px")
			 {
			   ...
			   if (index != -1)
			   {
				 ...
				 laser_corrections_[index].cosVertCorrection = std::cos (HDL_GrabberDriver_toRadians(laser_corrections_[index].verticalCorrection));
				 laser_corrections_[index].sinVertCorrection = std::sin (HDL_GrabberDriver_toRadians(laser_corrections_[index].verticalCorrection));
			   }
			 }
		   }
		 }
...
void HDLGrabberDriver::loadScanInterpolationCorrectionData(){
   ...
   scan_data_count_=0;
	 ...
	 if(sweepCorrectionFile_.empty())	 {
		 std::cerr<<"Error: sweep corrections file name is empty"<<std::endl;
	 }else{
	 	 std::cerr<<"sweep corr: "<< sweepCorrectionFile_<<std::endl;
		 scanFile_=fopen(sweepCorrectionFile_.c_str(),"r");
		 if ( scanFile_!=NULL)
		 {

			 scan_correction_LUT_=(struct HDLScanInterpolation *)malloc(icount*100*sizeof(struct HDLScanInterpolation));

			  while( feof(scanFile_)==0){

					  if(scan_data_count_== (icount*100)){
						  icount++;
						  scan_correction_LUT_=(struct HDLScanInterpolation *)realloc(scan_correction_LUT_,icount*100*sizeof(struct HDLScanInterpolation));


					  }
                fscanf(scanFile_, "%u\t%f\t%f\t%f\t%f\t%f\t%f\t%f\t%f\t%f\t%f\t%f\t%f\t%f\n",
                  &scan_correction_LUT_[scan_data_count_].sweepcounter,&scan_correction_LUT_[scan_data_count_].dt_scan,
                  ...
                  &scan_correction_LUT_[scan_data_count_].ppyaw[0],&scan_correction_LUT_[scan_data_count_].ppyaw[1],
                  &scan_correction_LUT_[scan_data_count_].pppitch[0],&scan_correction_LUT_[scan_data_count_].pppitch[1],
                  &scan_correction_LUT_[scan_data_count_].pproll[0],&scan_correction_LUT_[scan_data_count_].pproll[1],
                  &scan_correction_LUT_[scan_data_count_].ppz[0], &scan_correction_LUT_[scan_data_count_].ppz[1]);
				scan_data_count_++;
			  }
			scan_correction_LUT_=(struct HDLScanInterpolation *)realloc(scan_correction_LUT_,scan_data_count_*sizeof(struct HDLScanInterpolation));
			fclose( scanFile_);
            scanFile_ = NULL;
...
void HDLGrabberDriver::setTransformationParameters(double x,double y, double z, double roll, double pitch, double yaw)
{
...
bool HDLGrabberDriver::getParamsForFiringCorrection(unsigned int scancounter,float azimuth_pos,struct HDLScanInterpolation* params, struct HDLFiringCorrection *firing_params)
{
...
//	printf("Azimuthal position %f\n",azimuth_pos);
	...
	float speedxy[2] = {(params->ppx[1]-params->ppx[0])/params->dt_scan,(params->ppy[1]-params->ppy[0])/params->dt_scan};
	float speed;
...
//          timeSyncFile_ =fopen("pcapVelodyneTime.txt","w");
	...
	if(scan_correction_LUT_==NULL)
	{
		...
		std::cerr<<" Correction if OFF"<<std::endl;
	}else{
 ...
 // firecount++;
  ...
  if(firecount==0){ // cambiar !!
      ...
      if(  velodyneTimef1 > dataPacket->gpsTimestamp)
             std::cout<<"POSIBLE ERROR time stamp velodyne: "<< (double)dataPacket->gpsTimestamp-(double)velodyneTimef1<<std::endl;
	  ...
	  if(is_scan_correction_on_){
         ...
         if(stampcounter > dataPacket->gpsTimestamp)
                 std::cout<<"POSIBLE ERROR time stamp velodyne: "<< (double)dataPacket->gpsTimestamp-(double)stampcounter<<std::endl;
      ...
      //                 }
       ...
       if(firecount==0){ // cambiar !!
           if(  velodyneTimef1 > dataPacket->gpsTimestamp)
              ...
              if(is_scan_correction_on_){
                      getDataForScanCorrection(sweepCounter, &scan_correction_params_);
                      getParamsForFiringCorrection(firecount,(firingData.rotationalPosition)/100,&scan_correction_params_,&firing_correction_params_); //do correction numscan*0.533msec, numscan++
              }

              if(stampcounter > dataPacket->gpsTimestamp)
                     std::cout<<"stampCounter POSIBLE ERROR time stamp velodyne: "<< (double)dataPacket->gpsTimestamp-(double)stampcounter<<std::endl;
  ...
  for (int i = 0; i < HDL_FIRING_PER_PKT; ++i) //12
  {
    ...
    if (i < (HDL_FIRING_PER_PKT - 1)) {
      azimuth_diff = static_cast<float>(
          (36000 + dataPacket->firingData[i + 1].rotationalPosition - dataPacket->firingData[i].rotationalPosition) %
          36000);
    ...
    for (int j = 0; j < HDL_LASER_PER_FIRING; j++) // 32
    {
       ...
       if (azimuth_corrected < last_azimuth_)
      {
       ...
       if(firecount==0)
       {
              ...
              if(is_scan_correction_on_)
              {
                      getDataForScanCorrection(sweepCounter, &scan_correction_params_); // from sweepcounter, reset numscan
              }
       ...
       computeXYZI (xyzi, (int)(azimuth_corrected) /*firingData.rotationalPosition*/, dataPacket->firingData[i].laserReturns[j], laser_corrections_[j ]);
      ...
      if (std::isnan (xyz.x) || std::isnan (xyz.y) || std::isnan (xyz.z))
      {
       ...
       if(is_scan_correction_on_)
       {
       	   getParamsForFiringCorrection(firecount,((float)azimuth_corrected/*firingData.rotationalPosition*/)/100,&scan_correction_params_,&firing_correction_params_); //do correction numscan*0.533msec, numscan++
    	   doScanCorrection(xyzi, &firing_correction_params_);
       }
...
void HDLGrabberDriver::computeXYZI (pcl::PointXYZI& point, int azimuth, HDLLaserReturn laserReturn, HDLLaserCorrection correction)
{
...
// min_distance_threshold_ =  2.0;
  ...
  {
    double azimuthInRadians = HDL_GrabberDriver_toRadians ((static_cast<double> (azimuth) / 100.0) - correction.azimuthCorrection);
...
void HDLGrabberDriver::processGPSIMUPackets (){
	...
	// from velodyne_ros_driver gps_imu branch
		...
		for (int i=0; i<3;i++)
		{
			hdl_imu_data_->gyro_temp_accel[i].gyro = ((float)convert12bit2int16(dataPacket->gyro_temp_accel[i].gyro)) * 0.09766 *M_PI/180.0;
			hdl_imu_data_->gyro_temp_accel[i].temp = ((float)convert12bit2int16(dataPacket->gyro_temp_accel[i].temp)) * 0.1453+25;
			hdl_imu_data_->gyro_temp_accel[i].accel_x = ((float)convert12bit2int16(dataPacket->gyro_temp_accel[i].accel_x)) * 0.001221;  
			hdl_imu_data_->gyro_temp_accel[i].accel_y = ((float)convert12bit2int16(dataPacket->gyro_temp_accel[i].accel_y)) * 0.001221;  
...
void HDLGrabberDriver::readGPSPacketsFromSocket (){
	...
	/*		std::cout << "\n\nGiroscopo Gyro1 " << Giro1 << " Temp1 " << ((float)((0x0fff) & dataIMUPacket->Temp1))*0.1453+25 << " Acel1X " <<  ((float)((0x0fff) & dataIMUPacket->Acel1X))*0.001221 << " Acel1Y " << ((float)((0x0fff) & dataIMUPacket->Acel1Y))*0.001221 << std::endl;
		...
		*/
		if (isAddressUnspecified (source_address_filter_) ||
	        (source_address_filter_ == sender_gps_endpoint.address () && source_port_filter_ == sender_gps_endpoint.port ()))
		{
                      ...
                      /*  struct timeval captureTime;
                        ...
                        if((header->len - 42) == 1206) // laser
                        {
                            ...
                            if(timeSyncFile_!=NULL)
                            {
                                ...
                                fprintf(timeSyncFile_,"%d\t%llu\t%u\n",0,(unsigned long long)(auxTimeMS),packet_data->gpsTimestamp);
                            }
                            // write time stamp
...
void HDLGrabberDriver::fireCurrentSnapSweep (const unsigned short startAngle, const unsigned short endAngle,unsigned int sweepCount){
